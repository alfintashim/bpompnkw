...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{URL::to('/tindak_lanjut')}}"><i class="fa fa-list-alt"></i> Tindak Lanjut Aduan</a></li>
                        ...
                        <div class="tab-pane active" id="tab_1">
                            <div class="box-header with-border">
                                <div class="pull-left btn-group">
                                    <button type="button" class="btn bg-navy margin" data-toggle="modal" data-target="#modal-balas">
                                        <i class="fa fa-reply"></i> Balas 
                                    </button> 
                                    <button type="button" class="btn bg-navy margin" data-toggle="modal" data-target="#modal-disposisi">
                                        <i class="fa fa-mail-forward"></i> Disposisi 
                                    </button>
                                </div>
                            </div>
                        </div>
...
<!-- BALAS PESAN -->
<form role="form" method="POST" action="{{ URL::to('/tindak_lanjut/'.$detail_aduan->id.'/balas') }}" enctype="multipart/form-data">
    {{ csrf_field() }}
    {{ method_field('PUT') }}
    <div class="modal fade" id="modal-balas">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">Balas Langsung</h4>
                </div>
                <div class="modal-body">    
                    <div class="form-group">
                        <label>Status</label>
                        <input value="DISPOSISI" name="status_balas" class="form-control" readonly>
                    </div>

                    <div class="form-group">
                        <label>Ditujukan kepada</label>
                        <input value="Bidang Informasi dan Komunikasi" name="bidang_balas" class="form-control" readonly>
                    </div>

                    <div class="form-group">
                        <label>Isi Pesan</label>
                        <textarea class="form-control" name="isi_balas"></textarea>
                    </div>
                </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                    <h4 class="modal-title">Form Tindak Lanjut</h4>
            </div>
            <div class="modal-body">
                <div class="box-body">
                    <form role="form">
                ...
                <div class="box-body">
                    ...
                    <form role="form" method="POST" action="{{ URL::to('/tindak_lanjut/'.$detail_aduan->id.'/disposisi') }}" enctype="multipart/form-data">
                        {{ method_field('PUT') }}
                        {{ csrf_field() }}

                        <div class="form-group">
                            <label>Pengaduan</label>
                            <input value="{{ $detail_aduan->isi }}" class="form-control" readonly>
                        </div>
                        <div class="form-group">
                            <label>Keterangan</label>
                            <br>
                            {{ $detail_aduan->info_lain }}
                        </div>

                        <div class="form-group">
                            <label>Status</label>
                            <input value="DISPOSISI" name="status" class="form-control" readonly>
                        </div>

                        <div class="form-group">
                            <label>Ditujukan kepada</label>
                            <select class="form-control select2" style="width: 100%;" name="bidang">
                            	@if($detail_aduan->isi == 'Produk Kadaluarsa')
									<option>Bidang Pemeriksaan</option>
                                    <option>Bidang Penindakan</option>
                            	@elseif($detail_aduan->isi == 'Pemalsuan Produk')
									<option>Bidang Pemeriksaan</option>
                                    <option>Bidang Penindakan</option>
                            	@elseif($detail_aduan->isi == 'Komposisi Produk')
									<option>Bidang Pemeriksaan</option>
                                    <option>Bidang Informasi dan Komunikasi</option>  
                            	@elseif($detail_aduan->isi == 'Registrasi Produk')
									<option>Bidang Informasi dan Komunikasi</option> 
                            	@elseif($detail_aduan->isi == 'Produk Tidak Terdaftar')
                            		<option>Bidang Informasi dan Komunikasi</option> 
                            	@elseif($detail_aduan->isi == 'Penyalahgunaan Produk')
									<option>Bidang Pemeriksaan</option>
                                    <option>Bidang Penindakan</option>
                            	@endif 
                            </select>
                        ...
                        <div class="form-group">
                            <label>Untuk Melakukan</label>
                            <textarea class="form-control" name="note_disposisi"></textarea>
                        </div>
                </div>
            </div>
            <div class="modal-footer">
